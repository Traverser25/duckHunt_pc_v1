...
class BaseState(object):
    ...
    def renderNotices(self):
        ...
        font = pygame.font.Font(FONT,int(adjheight(20)))
...
class PlayState(BaseState):
    ...
    def execute(self, event):
        ...
        if event.type==SERIAL_EVENT:
            data=str((event.message))
            x,y,gundata=data.split(",")
            x=x.split('.')[0] if '.' in x else x
            y=y.split('.')[0] if '.' in y else y
            ...
            gundata=int(gundata)
            print(gundata)
            self.gun.moveCrossHairs((int(x),int(y)))
            if gundata == 0:
                hasFired = self.gun.shoot()
                for duck in self.ducks:
                    if hasFired and duck.isShot((x,y)):
